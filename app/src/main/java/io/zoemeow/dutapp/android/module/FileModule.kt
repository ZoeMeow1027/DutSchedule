...
class FileModule(
    ...
    fun getCacheNewsGlobal(): NewsCacheGlobal {
        ...
        try {
            file.bufferedReader().apply {
                ...
                val newsCacheGlobal = Gson().fromJson<NewsCacheGlobal>(text, (object : TypeToken<NewsCacheGlobal>() {}.type))
                this.close()
    ...
    fun getAccountSettings(): AccountSession {
        ...
        try {
            file.bufferedReader().apply {
                val text = this.use { it.readText() }
                val accountSession = Gson().fromJson<AccountSession>(text, (object : TypeToken<AccountSession>() {}.type))
                this.close()
